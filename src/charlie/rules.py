...
def _generate_merged_rules(
    ...
    frontmatter = {}
    # Check for old structure with sections
    ...
    # Use metadata from new structure if available
    elif config.rules and config.rules.metadata:
        frontmatter = config.rules.metadata

    title = config.rules.title if config.rules else "Development Guidelines"
    include_commands = True  # Default value since field was removed
    preserve_manual = True  # Default value since field was removed
    ...
    # Check if rules has sections (from old structure) or use prompt from new structure
    if config.rules:
        if hasattr(config.rules, "sections") and config.rules.sections:
            sorted_sections = sorted(
                config.rules.sections, key=lambda s: (s.order if s.order is not None else 999, s.title)
            )
            for section in sorted_sections:
                ...
                lines.append("")
        elif config.rules.prompt:
            # Use the new prompt-based structure
            content = transformer.transform(config.rules.prompt)
            lines.append(content)
            lines.append("")
...
def _generate_separate_rules(
    ...
    # Handle both old structure (sections) and new structure (prompt-based)
    if hasattr(config.rules, "sections") and config.rules.sections:
        ...
        # Handle new prompt-based structure - create a single rules file
        rules_path = Path(output_dir) / agent_spec.rules_file
        rules_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        if config.rules.metadata:
            content_parts.append(_format_frontmatter(config.rules.metadata))

        content_text = transformer.transform(config.rules.prompt)
        lines = [
            f"# {config.rules.title}",
            "",
            "Auto-generated by Charlie from configuration",
            f"Last updated: {datetime.now().strftime('%Y-%m-%d')}",
            "",
        ...
        rules_path.write_text(content, encoding="utf-8")
        generated_files.append(str(rules_path))
...
def generate_rules_for_agents(
    ...
    if mode == "separate" and config.rules and hasattr(config.rules, "sections") and config.rules.sections:
